...
/**
 * Stress IT: many tiny files → Bob upload → share Alice (Google).
 *
 * Env:
 ...
 *   ALTASTATA_IT_OWNER_ACCOUNT=google.rsa.bob123
 *   ALTASTATA_IT_READER_ACCOUNT=google.rsa.alice222
 *   ALTASTATA_IT_CLOUD_PREFIX=MANYSMALL
 *   ALTASTATA_IT_WAIT_SEC=7200
 *   RUN_MANY_SMALL_FILES_KEEP=1
 */
@RunWith(classOf[JUnitRunner])
class ManySmallFilesShareGoogleITSpec extends AnyFunSuite {
  ...
  private def waitUntil(label: String, timeoutSec: Long, onTick: () => Unit)(predicate: => Boolean): Unit = {
    ...
    while ({ last = predicate; !last } && System.nanoTime() < deadline) {
      ...
      if (now - lastTick > TimeUnit.SECONDS.toNanos(30)) {
        onTick()
        lastTick = now
        println(s"MANY_SMALL still waiting: $label")
      }
      Thread.sleep(2000L)
    }
  ...
  private def generateTinyFiles(count: Int, bytesPerFile: Int): File = {
    val tmp = Files.createTempDirectory("altastata-many-small-google").toFile
    ...
    while (i < count) {
      ...
      i += 1
      if (i % 1000 == 0) println(s"MANY_SMALL generated $i/$count local files")
    }
    println(s"MANY_SMALL generated $count files × ${bytesPerFile}B under ${tmp.getAbsolutePath}")
    tmp
  }
  ...
  test("many tiny files upload then Bob shares with Alice on Google") {
    ...
    val ownerAccount = env("ALTASTATA_IT_OWNER_ACCOUNT", "google.rsa.bob123")
    val aliceAccount = env("ALTASTATA_IT_READER_ACCOUNT", "google.rsa.alice222")
    ...
    def refresh(label: String): Unit = {
      println(s"MANY_SMALL refreshSessions ($label)")
    ...
    try {
      ...
      println(s"MANY_SMALL start prefix=$cloudPrefix files=$fileCount bytesEach=$fileBytes waitSec=$waitSec")
      ...
      println(s"MANY_SMALL upload done: listed=$bobListed elapsedMs=$uploadMs (~${uploadMs / 1000.0}s)")
      ...
      println(s"MANY_SMALL share API done: results=${shareResults.size} elapsedMs=$shareApiMs")

      val tWait0 = System.nanoTime()
      waitUntil(s"alice lists >= $fileCount", waitSec, () => {
        refresh("wait-alice")
        val n = listedCount(alice, cloudPrefix)
        println(s"MANY_SMALL alice progress: listed=$n / $fileCount")
      }) {
      ...
      println(s"MANY_SMALL alice saw $aliceListed file(s) waitMs=$waitMs")

      val strange = strangeMsgs(bob) ++ strangeMsgs(alice)
      strange.take(30).foreach(m => println(s"  STRANGE: ${m.replace("\n", " ").take(240)}"))
      if (strange.nonEmpty) {
        println(s"MANY_SMALL WARN: ${strange.size} Strange message(s)")
      } else {
        println("MANY_SMALL OK: no Strange change messages")
      }

      println(
        s"MANY_SMALL SUMMARY files=$fileCount bobListed=$bobListed aliceListed=$aliceListed " +
