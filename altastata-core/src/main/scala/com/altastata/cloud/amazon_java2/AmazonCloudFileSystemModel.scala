...
class AmazonCloudFileSystemModel(bucketName: String)(implicit account: Account) extends FileSystemModel {
  ...
  override def listCloudFiles(prefix: String, useFlatBlobListing: Boolean, startAfter: String = null, endBefore: String = null, mergeVersions: Boolean = true): java.util.Iterator[CloudFile] = {
    try {
      amazonS3Manager.getAmazonListWithDetails(bucketName, prefix).asScala.map (account.getFileSystemHandler().parseStorageFilePath(_)).iterator.asJava
    }
    catch {
  ...
  def storeLocalFileFuture()(inputFiles: Tuple2[File, CloudFile]): Future[CloudFileOperationStatus] = Future {    
    ...
    val result: Try[String] = 
      amazonS3Manager.storeInAmazon(bucketName, inputFiles._1, storageFilePath)
        
    result match {
      case Success(t) => {
        inputFiles._2.setOperationStateValue(OperationState.NONE)
        
        //Billing.sendBillingMsg("STS", inputFiles._1.length)
  ...
  def retrieveCloudFileFuture(baseDir: String, outputDir: String)(cloudFile: CloudFile): Future[CloudFileOperationStatus] = Future {    
    ...
    logger.debug(s"retrieveCloudFileFuture: ${outputFilePath}")
    
    cloudFile.setOperationStateValue(OperationState.DOWNLOADING)

    val result: Try[String] = 
      amazonS3Manager.retrieveFromAmazon(bucketName, new File(outputFilePath), cloudFile.getPath)
...
//    }
  ...
  def deleteCloudFileFuture(cloudFile: CloudFile): Future[CloudFileOperationStatus] = Future {
    ...
    try {
      ...
      val result: Unit = 
        amazonS3Manager.deleteObjectFromAmazon(bucketName, cloudFile.getPath)
